...
async function doResolve(apiKey: string): Promise<ResolveResult> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === INVALID_KEY_STATUS && isJson) {
        try {
          const parsed = responseText ? JSON.parse(responseText) as { valid?: boolean } : undefined;
    ...
    if (data.valid !== true) {
      console.error(`Key service returned malformed success payload for key ${shortKey}...`);
      return { ok: false, reason: 'malformed_response' };
    }

    const creds = data.credentials;
    if (!creds?.apiKey) {
      console.error(`Key service returned incomplete credentials for key ${shortKey}... (missing apiKey)`);
      return { ok: false, reason: 'malformed_response' };
    }
